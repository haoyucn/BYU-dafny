...
//      if (ch == EOL) { line++; col = 0; }
  ...
  public void CheckLiteral() {
		switch (t.val) {
			...
			case "expect": t.kind = 94; break;
			case "include": t.kind = 98; break;
			case "abstract": t.kind = 99; break;
			case "protected": t.kind = 100; break;
			case "module": t.kind = 101; break;
			case "refines": t.kind = 102; break;
			case "opened": t.kind = 103; break;
			case "provides": t.kind = 104; break;
			case "reveals": t.kind = 105; break;
			case "extends": t.kind = 106; break;
			case "new": t.kind = 107; break;
			case "nameonly": t.kind = 108; break;
			case "yields": t.kind = 109; break;
			case "returns": t.kind = 110; break;
			...
			case "for": t.kind = 121; break;
			case "match": t.kind = 122; break;
    ...
    switch (state) {
      ...
      case 0: {
        ...
        t.kind = recKind; break;
			...
			case 67:
				recEnd = pos; recKind = 1;
				if (ch == 39 || ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 67;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 68:
			...
			case 73:
				...
				else if (ch == '0') {AddCh(); goto case 78;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 74:
				recEnd = pos; recKind = 1;
				if (ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 79;}
			...
			case 77:
				...
				if (ch == 39 || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 67;}
			...
			case 81:
				...
				else if (ch == 'y') {AddCh(); goto case 82;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 82:
				recEnd = pos; recKind = 5;
				if (ch == 39 || ch == '0' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 6;}
				else if (ch == '1') {AddCh(); goto case 83;}
				else if (ch >= '2' && ch <= '9') {AddCh(); goto case 84;}
